...
// just whatever fits
// given the address of the location
char *get_label_name(int location_i)
{
	char *name = (char *)malloc(32 * sizeof(char));
	sprintf(name, "loc_%04x", locations[location_i]);
	return name;
}
...
{
	if(!find && comments) printf("\t\t; $%04x: %s", addr+base, format_n_instruction_bytes(n));
}
...
{
	int label_i;
	if((label_i = address_is_label(word+base)) != -1)
	{
...
void deal_with_labels()
{
	...
	int label_i;
	if((label_i = address_is_label(addr+base)) != -1)
	{
...
{
	// init the options
	base = 0; //base offset is none by default
